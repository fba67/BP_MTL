...
cell.name <- args[5] ## Either K562 or K562, at the moment
...
load("K562_single_cell_expr_swapped_transcript.RData")
...
for(i in seq(k)){
  ...
  outer_CV_partitions <- get.outerCV.partitions(features_i,response_i,n.folds=outerCV_folds)

  partitioned.data <- partition(x=features_i,y=response_i,percent=0.9) ## Partition the whole data defined above into training and test sets
  if(nrow(partitioned.data$train$x) < 50){
    nfold <- nrow(partitioned.data$train$x) - 1
  }else{
    nfold <- 10
  }
  ...
  cv.models <- mclapply(alpha_seq,function(alpha){
    cv.glmnet(x=as.matrix(partitioned.data$train$x),y=as.matrix(partitioned.data$train$y),family="mgaussian",parallel=T,alpha=alpha,nfolds=nfold)
  ...
  glmnet_res <- glmnet(x=as.matrix( partitioned.data$train$x),y=as.matrix( partitioned.data$train$y),family="mgaussian",lambda=cv.models[[best.model.idx]]$lambda.min,alpha=best.alpha) ## Perform the MTL on the training set using the optimal parameters lambda and alpha

  pred.res <- predict(glmnet_res,as.matrix(partitioned.data$train$x)) ## Get the prediction on the training set
  pred.res.test <- predict(glmnet_res,as.matrix(partitioned.data$test$x)) ## Get the prediction on the test set


  ## Compute covariances
  train.cov <- cov(as.matrix(partitioned.data$train$y), pred.res[,,1])
  test.cov <- cov(as.matrix(partitioned.data$test$y), pred.res.test[,,1])

  ## Compute RMSEs
  train.RMSE <- sqrt(sum((as.matrix(partitioned.data$train$y) - pred.res[,,1])^2) / length(pred.res))
  test.RMSE <- sqrt(sum((as.matrix(partitioned.data$test$y) - pred.res.test[,,1])^2) / length(pred.res.test))
  ...
  y.vectorized <- as.vector(as.matrix(partitioned.data$train$y))
  cor.train <- round(cor(pred.res.vectorized,y.vectorized),3)
  train.diff <- pred.res[,,1] - as.matrix(partitioned.data$train$y)

  ## Test
  pred.res.test.vectorized <- as.vector(pred.res.test[,,1])
  y.test.vectorized <- as.vector(as.matrix(partitioned.data$test$y))
  cor.test <- round(cor(pred.res.test.vectorized,y.test.vectorized),3)
  test.diff <- pred.res.test[,,1] - as.matrix(partitioned.data$test$y)

  ### Compute correlation per task
  ## Train
  train.cors <- sapply(seq(ncol(pred.res[,,1])),function(i)cor(pred.res[,i,1], partitioned.data$train$y[,i]))

  ## Test
  test.cors <- sapply(seq(ncol(pred.res.test[,,1])),function(i)cor(pred.res.test[,i,1], partitioned.data$test$y[,i]))
  ...
  indiv.preds_i <- train.indiv.models(features_i,response_i,alpha_seq) ## Get the predictions for each cell
  indiv.betas.mat <- NULL; ## To aggegates the beta coefficients spitted from the individual models

  print(length(indiv.preds_i$all.glmnet.res))

  ## assign the coefficients to indiv.betas.mat
  for(cell in seq(length(indiv.preds_i$all.glmnet.res)))
    indiv.betas.mat <- cbind(indiv.betas.mat,as.numeric(indiv.preds_i$all.glmnet.res[[cell]]$beta))
  ...
  pdf(paste("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/scMTL_hclust_",k,"_cluster_",i,"_response_",response.name,".pdf",sep=""))
  if(sum(betas) == 0){
    print(paste("the MTL for cluster",i,"was not successfully trained. All the coefficients are zero!"))
    next;
  }
  heatmap.2(betas,Rowv=F,Colv=F,dendrogram="n",trace="n",col=my_palette1,key=T,cexRow=.5,main=paste("MTL coefficients\n clust_",i,"_lambda*=",round(cv.models[[best.model.idx]]$lambda.min,3)," alpha*=",best.alpha,sep=""),labRow=colnames(features))
  ###
  ...
  ###
  ggplot(data=df,aes(x=cell,y=cor,fill=factor(dataset))) + geom_bar(stat="identity",position="dodge") + ggtitle(cell.name) + theme(plot.title=element_text(hjust=.5))
  if(F){
  ...
  write.table(glmnet_res$a0,paste("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/scMTL_a0_",i,"_response_",response.name,".txt",sep=""))
  all.betas[[i]] <- betas
  indiv.preds[[i]] <- indiv.preds_i
  dev.off()
}
save(indiv.preds,models,all.betas,file=paste("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/scMTL_hclust_",k,"_response_",response.name,".RData",sep=""))
